#!/bin/sh
############################################################################
#*项目名称:	  rmfp_bh_umri001c_apply表(oracle的ods层)推送到rmfp_bh_umri001c_apply表(hive)
#*作业名:		  rmfp_bh_umri001c_apply
#*创建时间:	  2021-10-14
#*负责人:		  EX-YOUXUAN001
############################################################################
...
sqoop import	 -D mapreduce.job.queue.name=root.ph_rc_queue \
				       ...
				       --username rbdppsqpw \
				       --password 'lqaz!WSX' \
				       --table RMFPDATA.RMFPDATA.RMFP_BH_UMRI001C_APPLY \
				       --hcatalog-database ph_rc_ods \  #要导入到hive库的hive的库名
				       --hcatalog-table RMFP_BH_UMRI001C_APPLY \  #要导入到hive库的hive的表名
				       --m 2



task_run $? "RMFPDATA.RMFP_CBIT_CAR_POLICY_APPLY(oracle的ods) to RMFP_CBIT_CAR_POLICY_APPLY(hive)"
echo "#############script sqoop_rmfp_bh_umri001c_apply_ods_to_hive.sh successfully finished #############"